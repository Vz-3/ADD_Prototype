...
from src.specrnet import frontends
...
class FrontendSpecRNet(SpecRNet):
    ...
    def _compute_frontend(self, x):
        ...
        print(f"Shape after mono conversion: {x.shape}")

        # Apply frontend processing
        frontend = self.frontend(x)
        print(f"Shape after frontend processing: {frontend.shape}")

        # Ensure correct dimensions
        if frontend.ndim == 3:
            frontend = frontend.unsqueeze(1)  # Add channel dimension
        elif frontend.ndim == 5:
            frontend = frontend.squeeze(1)  # Remove extra dimension if 5D
        
        print(f"Final frontend shape: {frontend.shape}")
        return frontend
